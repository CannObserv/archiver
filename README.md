...
Set `ARCHIVER_REDIS_URL=redis://default:<password>@broker:6379/0` to enable the outbox publisher background task that drains `changes_outbox` rows to the `info.changes` Redis Stream. Unset → publisher is silently disabled (degraded local-dev mode). The `default:` username is load-bearing (archiver#195).
