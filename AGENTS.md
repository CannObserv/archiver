...
half. All three hook scripts are symlinks into `skills-vendor/` - never
re-copy one, never turn the committed `.skills/doctor.sh` into one, and never
un-wire a hook to hold a submodule - use `.skills/skills-pin`. Why each, plus
