...
`ix_changes_outbox_dead_lettered` - a partial index over `dead_lettered_at IS NOT NULL` - backs the new `dead_lettered_count` stats query; the table has no pruner, so an unindexed count would degrade to an ever-slower seq scan. On top of it: `src/core/changes/outbox_stats.py` (depth, oldest-live-row age, dead-lettered count, computed on request), a dashboard health badge at `/dashboard/health/outbox`, and a periodic "Outbox stats" journald line from the publisher drain loop (INFO, WARNING while any dead-lettered row exists). Deliberately not on `/health`, which stays unauthenticated and DB-free. The 2026-07-29 activation incident (5 poison rows found only by someone tailing the journal) is the motivating case.

## v4.16.3 (2026-08-20)

[sdk] **Docstring text only: an em dash in `EnvelopeResponse` became a hyphen** (archiver#178). No schema, field, route, or method change; the SDK surface is byte-identical apart from that one line of prose.
