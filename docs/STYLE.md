...
- `.notes-row`, `.notes-row__actions`, `.notes-readout` — the read-only-first notes row inside a detail header panel (domain detail, #176). `.notes-row` is a wrapping flex row: the readout (or, in edit mode, the `.form-textarea`) takes the free width, `.notes-row__actions` holds the inline Edit / Cancel+Save buttons at intrinsic width and drops below on narrow viewports. `.notes-readout` carries the same border, radius, and padding as the `.form-textarea` it swaps with — so flipping to Edit does not shift the panel — plus `white-space: pre-wrap` to preserve author line breaks.
