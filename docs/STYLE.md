...
- `.detail-row` - an **authored** row of the same `.detail-grid__item` cells, where `.detail-grid` is an auto-filled one. A wrapping flex row whose children take `flex: 1 1 12rem` plus `min-width: 0`, so a long value shrinks rather than forcing the row wider than its panel. Reach for it when the grouping carries meaning and the author picked which fields share a row - the Watcher panel splits status-at-a-glance from provenance that way (#181). Reach for `.detail-grid` when the fields are peers and packing them by width is fine.
...
- `.watch-panel`, `.watch-panel__header`, `.watch-panel__fields` - the InfoItem detail screen's Watcher panel (#181). `.watch-panel` is the surface itself, previously an inline style on `#watcher-section` that reached for a `var(--radius)` naming no token. `.watch-panel__header` pins the pause/resume toggle to the panel's top right with `justify-content: flex-end` rather than absolute positioning, so the control reserves space instead of overlapping the first row when that row wraps. `.watch-panel__fields` stacks the editable rows one per line beneath the read-only ones.
