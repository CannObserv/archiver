...
- `.entity-card`, `.entity-card__header`, `.entity-card__title`, `.entity-card__meta`, `.entity-card__actions`. The header is a flex row of **two** children: the title block, and an `.entity-card__actions` slot for controls owning the whole card - which is why such a control never needs a block row of its own (#185). `> :first-child` carries `min-width: 0` and `.entity-card__title` `overflow-wrap: anywhere`, because a flex item's `min-width: auto` floors it at its content's min-content width and an InfoSource title is a whole URL: without the pair, the action slot is pushed out of the card rather than the title narrowing. `> h2 { margin: 0 }` for a header naming a *section* instead of the page (the Source Specification editor) - the header owns the gap below itself.
