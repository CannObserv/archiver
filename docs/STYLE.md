...
- `.detail-row` - an **authored** row of the same `.detail-grid__item` cells, where `.detail-grid` is an auto-filled one: the author picks which fields share a row, so the grouping can carry meaning (#181). Steps **3 -> 2 -> 1** columns on the width the row *has*, via `repeat(auto-fit, minmax(min(100%, 14rem), 1fr))` - not viewport media queries, which cannot see the fixed sidebar and drew three columns into a ~555px content column. `min(100%, …)` drops the track to full width when the container is narrower than the floor, which is what makes the last step a single column rather than an overflow.
...
- `.field-row-group`, `.field-row__label`, `.field-row`, `.field-row__actions`, `.field-row__readout` - the same read-only-first shape at **single-line** height, generalised from the notes row for the Watcher panel's editable fields (#181). `.field-row-group` stacks the label above the row so the label does not move on the flip; `.field-row__label` is the quiet uppercase treatment `.notes-heading` uses, without being an `<h2>`. `.field-row__readout` matches `.form-select--sm` on border, radius, and padding, and both share `flex: 1 1 14rem; min-height: 32px` for the reason `.notes-row` shares a floor: the flip must not resize the panel. Use this family for a control that fits on one line and `.notes-row` for a textarea.
- `.watch-panel`, `.watch-panel__header`, `.watch-panel__fields` - the InfoItem Watcher panel (#181). `.watch-panel__header` anchors the pause/resume toggle to the top right inside corner **out of flow** (`absolute` against the panel's `relative`); in flow it was a block row of its own, costing a button's height and pushing the first row down. The price is a gutter: `.watch-panel__header + .detail-row` reserves `6rem` on the whole row rather than one cell, because which cell sits under the button changes with the column count and CSS cannot know it. `.watch-panel__fields` stacks the editable rows.
