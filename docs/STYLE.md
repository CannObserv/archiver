...
- `.detail-row` - an **authored** row of the same `.detail-grid__item` cells, where `.detail-grid` is an auto-filled one: the author picks which fields share a row, so the grouping can carry meaning (#181). A grid stepping **3 -> 2 -> 1** columns at 900px and 640px, every track `minmax(0, 1fr)`. The 0 floor is the point - a bare `1fr` floors a track at its content's min-width, and a badge does not break, so the row outgrew its panel instead of narrowing.
...
- `.watch-panel`, `.watch-panel__header`, `.watch-panel__fields` - the InfoItem Watcher panel (#181). `.watch-panel` is the surface. `.watch-panel__header` anchors the pause/resume toggle to the top right inside corner **out of flow** (`absolute` against the panel's `relative`); in flow it was a block row of its own, costing a button's height and pushing the first row down. The price is a gutter: `.watch-panel__header + .detail-row` reserves `6rem`, narrowing to that row's first cell once the row is 1-up and the rest are stacked below the button rather than beside it. `.watch-panel__fields` stacks the editable rows.
