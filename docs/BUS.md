...
`content.artifacts.dlq`: the drainer grant (archiver#238), not the drain loop's.
A DLQ never joins `trim_topics` - a MAXLEN cap ignores triage: past it, the
oldest dead letters go whether or not anyone read them.
