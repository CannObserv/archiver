# Archiver Dashboard - detail screen conventions

**The canonical anatomy of an entity detail page: its header, its cells,
its affordances, and the mutation and pagination rules every one of them
follows.** Split out of [UI.md](UI.md) when that file passed its context
budget - this section alone was 56% of it, and nothing about the URL map,
proxy-header auth or the HTMX swap patterns needs it loaded.

Sibling docs: [UI.md](UI.md) shared mechanics - [PAGES.md](PAGES.md)
per-page route inventory - [COMPONENTS.md](COMPONENTS.md) Alpine catalogue -
[STYLE.md](STYLE.md) tokens and component classes.
...
the control they size for: `.notes-row` for a textarea (#176), `.field-row`
inside a `.field-row-group` for a single-line control (#181). Both share a
`min-height` across the halves and match the readout's border, radius, padding
and font-size to the control, so nothing moves on the flip. Classes in
[STYLE.md](STYLE.md), components in [COMPONENTS.md](COMPONENTS.md). Three
obligations:

- **The control must represent its own stored value.** A `<select>` offering a
  curated subset of what the field accepts selects index 0 for anything outside
  it, so an untouched Save overwrites the value the readout just showed. Emit
  the stored value as an extra selected option and let the write path take it
  back (#181 CR).
- **One value, one label.** Format it once and pass it to both halves. Two
  formatters disagree the first time a value falls outside the vocabulary the
  newer one knows.
- **Pick the FOUC trade deliberately.** Inline `style="display:none;"` on the
  edit half only where no no-JS save path exists to strand
  (`settings/_api_key_row.html`); where the route answers a non-HTMX POST with a
  303, both halves render for a frame so Save survives without Alpine
  (`domains/_notes_partial.html`). See **HTMX mutations**.
...
assignment sections, whose `poll` drives the self-refresh (#212) - takes its
