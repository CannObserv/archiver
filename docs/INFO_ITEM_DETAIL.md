...
5. **Replicator** — two sub-sections:
   ...
   - *Replication Specs* — `info_items/_rep_spec_assignments.html` (wrapper `#ii-rep-spec-assignments`, heading `#ii-rep-spec-heading`): `data-table` of active `info_item_rep_specs` assignments plus an assign form (`filter-card`, `rep_spec_id` field). Rows (`_rep_spec_row.html`) carry six columns — Spec, Provider, Activated, **Replication**, Public URL, Actions. HTMX deactivate re-renders the whole section (table + empty state) and focuses the heading; a per-row **Replicate now** swaps the individual row.
