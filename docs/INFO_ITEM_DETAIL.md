...
"Every outcome flashes" also depends on the section's poll yielding to the click (archiver#220). Unsynced, a poll that landed first swapped the section out from under the action, and the action's response - swap, focus and this toast - was discarded. The operator saw nothing happen, on a fresh, enabled button, and a second click is a second occasion. Both actions now `hx-sync` on the section with `drop`, which aborts an in-flight poll; see UI.md § *A section that polls and takes actions*.
