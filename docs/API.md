...
| List a DLQ | `GET /tools/dead-letters/{dlq}` | generated only (operator control, archiver#238): paginated `{items, has_more, limit, offset}`, oldest first, each entry decoded against the running co-core; `{dlq}` outside `content.revisions.dlq` / `content.artifacts.dlq` is a 422, a dormant bus a 409, an unreadable broker a 503 |
| Discard DLQ entries | `POST /tools/dead-letters/{dlq}/discard` | generated only (operator control, archiver#238): `{"entry_ids": [...]}` (1-500 exact `<ms>-<seq>`, each half a uint64) → `{discarded, not_found}`; each frame logged to journald, then `XDEL`ed. Runbook: [BUS_CONSUMERS.md](BUS_CONSUMERS.md#triaging-the-two-dlqs-archiver238) |
