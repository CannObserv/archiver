...
| Discard DLQ entries | `POST /tools/dead-letters/{dlq}/discard` | generated only (operator control, archiver#238): `{"entry_ids": [...]}` (1-500 exact `<ms>-<seq>`) → `{discarded, not_found}`; each frame logged to journald, then `XDEL`ed. Runbook: [BUS_CONSUMERS.md](BUS_CONSUMERS.md#triaging-the-two-dlqs-archiver238) |
