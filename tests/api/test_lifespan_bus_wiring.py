...
async def test_trim_allowlist_is_info_changes_only(bus_env, bus_client_calls, test_engine):
    ...
    *command* stream deletes commands nobody delivered and orphans the PEL
    ...
    (archiver#141). Pinned as a literal so widening it is deliberate.
    ...
    broker assigns archiver to drain - the drainer's (archiver#238), which
    disposes with ``XDEL`` and issues no ``XTRIM``; they never join this set. Broker's
