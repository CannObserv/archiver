...
# against a live Archiver service.
#
# Reference flow:
...
#
# Requires:
#   - Archiver service on $ARCHIVER_BASE_URL (default http://localhost:8020).
#   - ARCHIVER_API_KEY in env (loaded automatically below from /etc/archiver/.env + .env).
#   - jq, curl.
...
KEY="${ARCHIVER_API_KEY:?ARCHIVER_API_KEY required}"
